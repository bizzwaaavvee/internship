...
async function createCustomCert() {
  const name = "MALEY SAI VIKRAM";
  const email = "2400031241@kluniversity.in";
  const password = "password123";
  const dob = "2006-08-02";
  const domain = "fsd";
  const college = "KL University"; // Based on email domain
  const regNo = "2400031241";
  ...
  const studentId = 'Stud-' + regNo; // Using reg no as student ID since it is available
  ...
  fs.writeFileSync('MALEY_SAI_VIKRAM_Certificate.pdf', pdfBytes);
  console.log("Certificate saved to MALEY_SAI_VIKRAM_Certificate.pdf");
}
