...
async function createCustomCert() {
  const name = "REDDIBOINA SAI VENU";
  const email = "Saivenureddiboina13@gmail.com";
  const password = "Saivenureddiboina13@gmail.com";
  const dob = "2006-05-01";
  const domain = "fsd";
  ...
  const created_at = new Date('2026-08-12T10:00:00Z').toISOString();
  ...
  // Custom Issue Date
  const issueDate = "August 12, 2026";
  ...
  fs.writeFileSync('REDDIBOINA_SAI_VENU_Certificate.pdf', pdfBytes);
  console.log("Certificate saved to REDDIBOINA_SAI_VENU_Certificate.pdf");
}
