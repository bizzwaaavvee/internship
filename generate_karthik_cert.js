...
async function createCustomCert() {
  const name = "KARTHIK BALAJI VEDURUPARTHI";
  const email = "karthikbalaji@gmail.com";
  const password = "karthikbalaji@gmail.com";
  ...
  const certId = 'Cert-VHS6C9LS0';
  const studentId = 'Stud-TFZ3E23YJ';
  const created_at = new Date('2024-07-20T10:00:00Z').toISOString();
  ...
  if (dbError) {
    ...
    // return; // Commented out to ensure PDF generation proceeds
  }
  ...
  const nameWidth = drawCenterText(name, height - 405, helveticaBold, 30, primaryColor);
  ...
  // Custom Issue Date
  const issueDate = "18th September 2026";
  ...
  fs.writeFileSync('KARTHIK_BALAJI_VEDURUPARTHI_Certificate.pdf', pdfBytes);
  console.log("Certificate saved to KARTHIK_BALAJI_VEDURUPARTHI_Certificate.pdf");
}
